export default function SchoolInfo({ data }) {
    ...
    return (
      <div style={{ margin: '20px', padding: '20px', border: '1px solid #ccc', borderRadius: '10px', backgroundColor: '#f9f9f9' }}>
        ...
        <p style={{ color: '#555' }}>Forecasted Change based only on demographics: <strong style={{ color: data.students_change_sidewalk >= 0 ? 'green' : 'red' }}>{formatChange(data.students_change_sidewalk)}</strong></p>
      </div>
    );
}
